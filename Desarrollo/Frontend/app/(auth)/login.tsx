...
export default function LoginScreen() {
  ...
  const onLogin = async ({ email, password }: LoginForm) => {
    try {
      ...
      const me = await AuthApi.login(email, password); // { id, email, fullName }
      setUser(me);                 // 👈 guarda al usuario en el contexto
      router.replace('../(app)/home');     // 👈 navega al Home
    } catch (e: any) {
      // ...tu manejo de errores (401, etc.)
    } finally {
