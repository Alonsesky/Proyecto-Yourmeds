...
export default function LoginScreen() {
  ...
  const onLogin = async (values: LoginForm) => {
    try {
      ...
      // 1) token devuelto por el backend: "Bearer eyJh..."
      const rawToken = data?.token;
      if (!rawToken) {
        throw new Error('El backend no devolvió un token.');
      }

      // 2) Dejamos solo el JWT sin el prefijo "Bearer "
      const token = rawToken.startsWith('Bearer ')
        ? rawToken.slice('Bearer '.length)
        : rawToken;

      // 3) Guardar el id de usuario para snapshots
      await saveUserId(data.user.id);

      // 4) Guardar token en sesión (AsyncStorage + AuthContext)
      await signIn(token);
      ...
      setErrorDetails(backendMessage);
      setErrorOpen(true);
    } finally {
